...
void renderer_init() {
  ...
  glVertexAttribPointer(0, 3, GL_SHORT, GL_FALSE, sizeof(Vertex3D),
                        (const void *)offsetof(Vertex3D, x));
  glVertexAttribPointer(1, 3, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(Vertex3D),
                        (const void *)offsetof(Vertex3D, r));
  glVertexAttribPointer(2, 2, GL_UNSIGNED_BYTE, GL_FALSE, sizeof(Vertex3D),
                        (const void *)offsetof(Vertex3D, u));
...
void renderer_draw_mesh_shaded(const Mesh *mesh, Transform *model_transform) {
  ...
  // Copy data into it
  glBufferData(GL_ARRAY_BUFFER, mesh->n_vertices * sizeof(Vertex3D),
...
void renderer_draw_triangles_shaded_2d(const Vertex2D *vertex_buffer,
