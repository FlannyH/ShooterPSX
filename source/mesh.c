...
model_t* model_load(const char* path, int on_stack, stack_t stack) {
    ...
    uint16_t* index_buffer = (uint16_t*)(((intptr_t)binary_section) + model_header->offset_index_data);
    polygon_metadata_t* poly_metadata = (polygon_metadata_t*)(((intptr_t)binary_section) + model_header->offset_poly_metadata);
    ...
    for (size_t i = 0; i < model_header->n_submeshes; ++i) {
        ...
        model->meshes[i].indices = &index_buffer[mesh_descriptions[i].index_start];
        model->meshes[i].polygon_metadata = &poly_metadata[mesh_descriptions[i].polygon_metadata_start];
