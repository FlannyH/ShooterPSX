...
void state_update_debug_menu_main(scalar_t dt) {
	...
	for (int i = 0; i < 3; ++i) {
		...
		if (i == state.debug_menu_main.button_selected) {
			if (state.debug_menu_main.button_pressed) {
				color.r = 64;
				color.g = 64;
				color.b = 64;
			}
		...
		renderer_draw_text((vec2_t){64*ONE, (96 + (24 * i))*ONE}, text_debug_menu_main[i+1], 1, 0, white);
		renderer_draw_2d_quad_axis_aligned((vec2_t){256*ONE, (96 + (24 * i))*ONE}, (vec2_t){448*ONE, 20*ONE}, (vec2_t){0*ONE, 144*ONE}, (vec2_t){192*ONE, 164*ONE}, color, 2, 5, TEX_CAT_MISC);
	}

	// Handle button navigation
	if (input_mapping_pressed(IM_MENU_UP, 0) && state.debug_menu_main.button_selected > 0) {
		state.debug_menu_main.button_selected--;
		state.debug_menu_main.button_pressed = 0;
	}
	if (input_mapping_pressed(IM_MENU_DOWN, 0) && state.debug_menu_main.button_selected < 2) {
		state.debug_menu_main.button_selected++;
		state.debug_menu_main.button_pressed = 0;
	}
	if (input_mapping_pressed(IM_MENU_GO, 0)) {
		state.debug_menu_main.button_pressed = 1;
	}

	// Handle full button presses
